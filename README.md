...
- 🗂️ **Preloaded via `file://`** — Full skill definition available from the first message, no lazy-load delay
...
{
  "resources": [
    "file://<SKILL_PATH>/SKILL.md"   // file:// for preload at session start
  ],
  "hooks": {
...
> Use `file://` (not `skill://`) so the full SKILL.md is preloaded at session start — the agent needs mem.sh commands, event types, and graduation rules from the first message. The `agentSpawn` hook complements this by injecting runtime data (memory, pending logs, review reminders).
...
│   ├── skill-router.sh      # Legacy skill router (replaced by skill:// resources)
